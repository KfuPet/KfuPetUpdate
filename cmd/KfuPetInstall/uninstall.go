...
func uninstallKfuPet(installDir string, opts uninstallOptions) ([]string, error) {
	...
	if err := winreg.ClearInstallRecord(); err != nil {
		return warnings, fmt.Errorf("删除安装信息失败：%w", err)
	}
	if err := winreg.ClearUninstallEntry(); err != nil {
		return warnings, fmt.Errorf("删除卸载入口失败：%w", err)
	}
	return warnings, nil
}
