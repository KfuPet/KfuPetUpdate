...
`gen-manifest.ps1` 生成 `KfuPet-manifest.json`——压缩包 sha256（安装时校验安装包用）+ 随包发布的根级文件哈希（供后续的「修复」功能做逐文件校验）+ 本安装器自身的哈希与版本（供安装/升级/修复更新用户机上的常驻更新程序）；`Characters` 等子目录不收录。
...
清单里的 `installer` 段（名字、sha256、大小、版本）取自 `dist/KfuPetInstall.exe`，可用 `-Installer` 指定别的路径；该 exe 不能放在 `-Dir` 之内（它是随 Release 单独上传的附件，不属于随包发布的 KfuPet 文件）。所以**一次发布要传三个附件**：zip、清单、`KfuPetInstall.exe`；漏传 exe 或传了与清单对不上的那份，用户的更新程序就换不了（详见「更新程序自身的更新」）。
...
分发包名为 `KfuPetInstall.exe`；安装时按固定名 `KfuPetUpdate.exe` 在安装目录里放一份常驻副本（发布版带了更新的安装器就用那一份，否则就是复制自身，见「更新程序自身的更新」）：标准卸载入口与 KfuPet 的「立即更新」都指向这个固定位置（KfuPet 侧的更新约定认这个名字），用户删掉当初下载的 updater 也不影响后续卸载与升级。
...
升级复用安装流程：下载 → sha256 校验 → 解压到暂存目录 → 整目录替换（失败回滚）→ 放置常驻更新程序
...
更新程序（本程序）没有独立的更新通道：它常驻在安装目录、按固定名 `KfuPetUpdate.exe` 被桌宠的
「立即更新」和「应用和功能」里的卸载入口调用，用户不会专门去下载它的新版本。因此**它的更新搭
KfuPet 发布版的顺风车**——发布版里的 `KfuPetInstall.exe` 就是它的新版本，清单的 `installer`
段记着这一份的 sha256。

安装、升级、修复都会放置这个常驻副本（即步骤清单里的「正在更新更新程序」）：

| 情况 | 怎么放 |
| --- | --- |
| 发布版那一份与本机运行的这个逐字节相同 | 复制自身，不下载 |
| 不同，且发布版不比本机旧 | 从发布版下载 `KfuPetInstall.exe`、按清单 `installer.sha256` 校验后写进安装目录 |
| 不同，但发布版比本机这份旧 | 不下载（宁可不动，不能倒退），复制自身 |
| 取不到清单或发布信息（离线安装、无网） | 复制自身——至少保证常驻副本在位、能跑 |

判定分两步，缺一不可：

1. **比哈希定"是不是同一份文件"**：本机运行的这个 exe 算一遍 sha256，与清单里的
   `installer.sha256` 相同就什么都不做。常驻副本正是由它复制出来的，这一步命中是常态。
2. **比版本定方向**：哈希不同时再看版本，发布版比本机这份**旧**就不动；不低于本机才下载。

**只比版本是不够的**：同一个版本号可以有多个不同的构建——`app.rc` 不 bump（或改了忘了重新
生成 syso）时版本号不变，重新编译出来的就不是同一份文件。只靠版本判断，"版本没变、内容变了"
的新构建永远送不出去，而且不报错。哈希比对没有这个失效模式；版本则补上哈希单独给不出的两件事：
"谁更新"（方向，用来挡住降级）和报告里给人看的版本号文案。

本机版本读的是 exe 资源里的 `FILEVERSION`（`app.rc` 的版本信息，见 `internal/winapi/` 的
`FileVersion`），与清单 `installer.version` 取自同一处资源，因此不存在"代码里的常量忘了同步"
导致判断失准的问题；读不到时按"不旧"处理，宁可多下一次，也不让更新通道失效。

下载 47 MB 换来一个能自我更新的更新通道，代价是"更新程序有变化"的那次升级会多花点时间，
进度条会走「正在更新更新程序」这一步。**这一步失败只降级为警告**，不影响 KfuPet 的安装/升级
本身；拿不到新版时就退回复制自身，常驻副本不会因此缺失。

两个边界：

- 本程序正以常驻副本的身份运行时（用户直接运行安装目录里的 `KfuPetUpdate.exe`），目标就是
  自己的映像、写不进去，这一步会记一条提示留给下次（届时由别的实例来换）。
- **v2.0.0 及更早的常驻副本里没有这套逻辑**，它们只会把自身复制过去，因此现存用户需要最后
  手动下载一次 `KfuPetInstall.exe`——用新下载的这个跑一次「修复」即可，从那以后它就能自己更新了。
...
先体检。前三项通常能就地补上（常驻副本若落后于发布版则要下载一份新的），只有本体文件必然要下载：

| 体检项 | 判定 | 怎么补 |
| --- | --- | --- |
| 常驻副本 `KfuPetUpdate.exe` | 与清单里 `installer` 的 sha256 一致（既挡住被杀软截断的 0 字节文件，也能发现版本落后）；拿不到清单时只看存在性 | 换成发布版那一份，**可能要下载**；不需要下载时从自身复制 |
...
没有需要下载的项时几秒就完成。最常见的场景"常驻副本被删"就属于这一类——只要手上这个安装器正是
发布版那一份，补回它不用联网：此时桌宠的「立即更新」和「应用和功能」里的卸载都是坏的，
而修复不用联网就能补回来。只有需要覆盖 KfuPet 本体文件时
才会先拦住、要求用户退出——正在运行的 exe/dll 映像替换不掉；补常驻副本、快捷方式、安装信息
都不碰这些文件，就不会拦下用户。

修复**不提供"选本地安装包"的离线模式**：本体文件一律按线上发布版（GitHub 优先、Gitee 兜底）
校验与覆盖，本地自选的文件无从核对来源。
...
同一时刻只允许一个 updater 实例改动安装目录（命名互斥体 `Local\KfuPetUpdate-Singleton`）：两个实例同时
安装/卸载会争抢同一个安装目录，而且常驻副本正在运行时，别的实例既替换不了也删不掉它。

启动时若已有实例在跑，会先看它是不是**闲着**——每个实例在安装/修复/卸载/升级期间置位命名事件
`Local\KfuPetUpdate-Busy`，表示自己正在干活。旧实例闲着（只是个开着没动过的窗口）就直接结束掉它，
新实例接管；正在干活则提示「KfuPet 更新程序正在安装或修复中」并退出，绝不强杀正在进行中的流程，
否则会把安装目录留在装了一半的状态。
...
- `gen-manifest.ps1`：发布清单生成脚本（zip sha256 + 随包发布的根级文件哈希 + 安装器自身的哈希与版本（`-Installer`，缺省 `dist/KfuPetInstall.exe`）；zip 不存在时按目录自动打包，已存在的包不覆盖，随后核对目录与 zip 逐文件一致，不一致即报错）；**须保持 UTF-8 with BOM 编码**
- `cmd/KfuPetInstall/`：主程序（`package main`），唯一的编译入口；资源与图标必须与包放在同一目录，才能被链入 exe
  ...
  - `installer.go`：安装流程（下载 → 按哈希清单/发布信息校验 sha256 → 解压 → 替换安装目录 → 放置常驻更新程序 → 创建快捷方式 → 写注册表；运行环境排在 KfuPet 本体之前）
  - `updater.go`：更新程序自身的更新（按清单里的 `installer` 段判断常驻副本该不该换成发布版那一份，需要时下载并校验 sha256，失败降级为警告）
